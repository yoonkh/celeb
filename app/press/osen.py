...
TARGET_URL_BEFORE_PAGE_NUM = 'http://osen.mt.co.kr/search/?offset='
...
def get_link_from_news_title(page_num, URL, type, press):
    ...
    for i in range(page_num):
        current_page_num = i * 15
        position = URL.index('=')
        position2 = URL.index('&keyword=')
        URL_with_page_num = URL[: position + 1] + str(current_page_num) + URL[position2:]
        print(URL_with_page_num)
        source_code_from_URL = urllib.request.urlopen(URL_with_page_num)
        # print(source_code_from_URL)
        soup = BeautifulSoup(source_code_from_URL, 'lxml', from_encoding='utf-8')
        # print(soup)
        for title in soup.select('div.searchObj2 > span > a'):
            print(title)
            title_link = TARGET_URL_BEFORE_PAGE_NUM[:-16] + title['href']
            print(title_link)
